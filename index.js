...
async function generateContent(topic) {
  const res = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
    ...
    messages: [
      { role: 'system', content: 'You are an elite Instagram content creator. Analyze the topic and create the right post type. If it is a person name use their real famous quote. If it is a tool or tips topic create a list post. If motivational create a bold statement. Respond ONLY with raw JSON no backticks no markdown. Format: {"quote":"SHORT text MAX 15 WORDS for the image","author":"person name or empty","caption":"2-4 paragraphs engaging caption max 200 words including numbered list if tips post","cta":"call to action line","hashtags":"12 hashtags with # separated by spaces"}' },
  ...
  raw = raw.substring(s, e + 1).replace(/[\x00-\x08\x0b\x0c\x0e-\x1f]/g, ' ');
