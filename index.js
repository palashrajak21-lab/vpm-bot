...
// Upload image to Telegram and get a permanent file_id + URL
async function uploadToTelegram(chatId, imageBuffer) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
  form.append('document', imageBuffer, { filename: 'post.png', contentType: 'image/png' });
  const res = await axios.post(TELEGRAM_API + '/sendDocument', form, { headers: form.getHeaders() });
...
app.post('/webhook', async function(req, res) {
  ...
  try {
    ...
    if (lower === 'approve' && sessions[chatId]) {
      ...
      await sendText(chatId, 'Uploading and posting to Instagram...');
      const imageUrl = await uploadToTelegram(chatId, s.imgBuffer);
      const postId = await postToInstagram(imageUrl, s.caption + '\n\n' + s.cta + '\n\n' + s.hashtags);
