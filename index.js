...
async function postToInstagram(imgId, caption, client) {
  ...
  const create = await axios.post('https://graph.instagram.com/v21.0/' + client.igUserId + '/media', {
    image_url: imageUrl, caption: caption, access_token: client.igToken
  });
  ...
  const pub = await axios.post('https://graph.instagram.com/v21.0/' + client.igUserId + '/media_publish', {
    creation_id: create.data.id, access_token: client.igToken
  });
  return pub.data.id;
}
...
app.listen(process.env.PORT || 3000, () => console.log('VPM SaaS Bot running!'));

// ── Client Onboarding Page ────────────────────────────────────
// Clients visit this page, enter their bot token + click Connect Instagram

app.get('/connect', (req, res) => {
  const { bot, error } = req.query;
  res.send(`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .card{background:#0d1420;border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:48px 40px;width:100%;max-width:480px}
  .logo{text-align:center;margin-bottom:32px}
  .logo-text{font-size:24px;font-weight:700;color:#4da6ff}
  .logo-sub{font-size:14px;color:#8892a4;margin-top:4px}
  h1{font-size:22px;font-weight:700;margin-bottom:8px;text-align:center}
  .subtitle{font-size:15px;color:#8892a4;text-align:center;margin-bottom:32px;line-height:1.5}
  .steps{display:flex;flex-direction:column;gap:16px;margin-bottom:32px}
  .step{display:flex;align-items:flex-start;gap:16px;padding:16px;background:#060911;border-radius:12px;border:1px solid rgba(255,255,255,0.06)}
  .step-num{width:32px;height:32px;border-radius:50%;background:#4da6ff;color:#060911;font-weight:700;font-size:14px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
  .step-text{font-size:14px;line-height:1.5}
  .step-title{font-weight:600;margin-bottom:2px}
  .step-desc{color:#8892a4;font-size:13px}
  label{display:block;font-size:13px;font-weight:500;color:#8892a4;letter-spacing:0.5px;text-transform:uppercase;margin-bottom:8px}
  input{width:100%;background:#060911;border:1px solid rgba(255,255,255,0.12);border-radius:12px;padding:14px 16px;color:#e8eaf0;font-size:15px;font-family:monospace;outline:none;transition:border-color 0.2s;margin-bottom:20px}
  input:focus{border-color:#4da6ff}
  input::placeholder{color:#3a4455;font-family:inherit}
  .btn-ig{display:flex;align-items:center;justify-content:center;gap:12px;background:linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888);border:none;border-radius:12px;padding:16px;color:#fff;font-size:16px;font-weight:700;cursor:pointer;width:100%;text-decoration:none;transition:opacity 0.2s}
  .btn-ig:hover{opacity:0.9}
  .ig-icon{width:24px;height:24px;fill:#fff}
  .error{background:rgba(229,62,62,0.1);border:1px solid rgba(229,62,62,0.3);color:#fc8181;border-radius:10px;padding:12px 16px;margin-bottom:20px;font-size:14px}
  .divider{height:1px;background:rgba(255,255,255,0.08);margin:24px 0}
  .help{font-size:13px;color:#8892a4;text-align:center;line-height:1.6}
  .help a{color:#4da6ff;text-decoration:none}
</style>
</head>
<body>
<div class="card">
  ...
  <p class="subtitle">Set up your automated posting bot in 2 simple steps. No technical knowledge needed!</p>

  <div class="steps">
    <div class="step">
      <div class="step-num">1</div>
      <div class="step-text">
        <div class="step-title">Enter your Telegram Bot Token</div>
        <div class="step-desc">Get this from @BotFather on Telegram by creating a new bot</div>
      </div>
    </div>
    <div class="step">
      <div class="step-num">2</div>
      <div class="step-text">
        <div class="step-title">Connect Instagram</div>
        <div class="step-desc">Click the button below and log in with your Instagram account</div>
      </div>
    </div>
  </div>
  ...
  <form action="/connect/start" method="GET">
    <label>Your Telegram Bot Token</label>
    <input name="bot" placeholder="1234567890:AAHxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" required value="${bot || ''}">
    <label>Your Business Name</label>
    <input name="name" placeholder="e.g. Tech Startup India" required>
    <label>Your Instagram Handle</label>
    <input name="handle" placeholder="e.g. techstartupindia (without @)" required>
  ...
  <div class="help">
    Need help? Contact us at <a href="mailto:support@visualpromedia.com">support@visualpromedia.com</a>
  </div>
</div>
</body>
</html>`);
...
app.get('/connect/start', (req, res) => {
  const { bot, name, handle } = req.query;
  if (!bot || !name || !handle) return res.redirect('/connect?error=Please+fill+all+fields');

  // Store temp data in memory (expires in 10 min)
  const state = Buffer.from(JSON.stringify({ bot, name, handle, ts: Date.now() })).toString('base64');

  const IG_APP_ID = process.env.IG_APP_ID || '2019846002295877';
  const redirectUri = encodeURIComponent(PUBLIC_URL + '/connect/callback');
  const scope = 'instagram_basic,instagram_content_publish';
  const authUrl = `https://api.instagram.com/oauth/authorize?client_id=${IG_APP_ID}&redirect_uri=${redirectUri}&scope=${scope}&response_type=code&state=${state}`;
...
app.get('/connect/callback', async (req, res) => {
  ...
  if (error) return res.redirect('/connect?error=Instagram+connection+cancelled');
  if (!code || !state) return res.redirect('/connect?error=Invalid+response+from+Instagram');

  try {
    const stateData = JSON.parse(Buffer.from(state, 'base64').toString());
    const { bot, name, handle } = stateData;
    ...
    const longTokenRes = await axios.get('https://graph.instagram.com/access_token', {
      params: { grant_type: 'ig_exchange_token', client_secret: IG_APP_SECRET, access_token: shortToken }
    });
    ...
    const clients = loadClients();
    const id = 'client_' + Date.now();
    clients[id] = { id, name, handle, igUserId: String(igUserId), igToken: longToken, botToken: bot, active: true, createdAt: new Date().toISOString() };
    saveClients(clients);

    // Auto-set webhook
    await axios.get(`https://api.telegram.org/bot${bot}/setWebhook?url=${PUBLIC_URL}/webhook/${id}`);

    // Send welcome message to bot
    await axios.post(`https://api.telegram.org/bot${bot}/sendMessage`, {
      chat_id: (await axios.get(`https://api.telegram.org/bot${bot}/getUpdates`)).data.result?.[0]?.message?.chat?.id || '',
      text: `Welcome to ${name} Bot! Your Instagram @${handle} is now connected. Send me any topic to create a post!`
    }).catch(() => {});
...
.card{background:#0d1420;border:1px solid rgba(255,255,255,0.1);border-radius:24px;padding:48px 40px;width:100%;max-width:440px;text-align:center}
.icon{font-size:64px;margin-bottom:24px}h1{font-size:24px;font-weight:700;margin-bottom:12px;color:#00ba7c}
p{color:#8892a4;line-height:1.6;margin-bottom:8px}.handle{color:#4da6ff;font-weight:600}</style></head>
<body><div class="card">
<div class="icon">&#x2705;</div>
<h1>You're all set!</h1>
<p>Instagram <span class="handle">@${handle}</span> is now connected.</p>
<p style="margin-top:16px">Open your Telegram bot and send any topic to create your first post!</p>
<p style="margin-top:24px;font-size:13px;color:#3a4455">Your bot will automatically post to Instagram when you approve.</p>
</div></body></html>`);

  } catch(e) {
    console.error('OAuth error:', e.message, e.response?.data);
    res.redirect('/connect?error=' + encodeURIComponent('Connection failed: ' + (e.response?.data?.error_message || e.message)));
  }
});
