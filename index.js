...
async function postToInstagram(imgBuffer, caption) {
  ...
  const create = await axios.post('https://graph.instagram.com/v21.0/' + IG_USER_ID + '/media', { image_url: imageUrl, caption: caption, access_token: IG_TOKEN });
  const publish = await axios.post('https://graph.instagram.com/v21.0/' + IG_USER_ID + '/media_publish', { creation_id: create.data.id, access_token: IG_TOKEN });
  setTimeout(function() { try { fs.unlinkSync(filepath); } catch(e) {} }, 60000);
