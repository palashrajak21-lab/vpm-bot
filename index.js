...
async function postToInstagram(imgId, caption) {
  // Verify image is accessible before posting to Instagram
  const imageUrl = PUBLIC_URL + '/img/' + imgId + '.jpg';
  console.log('Verifying image URL:', imageUrl);
  // Ping the URL to make sure server is awake and image exists
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const check = await axios.head(imageUrl, { timeout: 5000 });
      if (check.status === 200) { console.log('Image accessible!'); break; }
    } catch(e) {
      console.log('Waiting for image to be accessible, attempt:', attempt + 1);
