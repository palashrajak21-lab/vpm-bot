...
setTimeout(refreshAllTokens, 30000);
  ...
  `<div class="clients-grid">${clientList.map(c => `
    <div class="client-card">
      <form method="POST" action="/admin/delete" style="display:inline">
        ...
        <input type="hidden" name="password" value="${req.query.p || ''}">
        <a href="/admin/edit?p=${req.query.p || ''}&id=${c.id}" style="position:absolute;top:16px;right:170px;background:rgba(77,166,255,0.1);border:1px solid rgba(77,166,255,0.2);color:#4da6ff;border-radius:8px;padding:6px 12px;font-size:12px;text-decoration:none;font-family:inherit">Edit</a>
        <a href="/admin/refresh?p=${req.query.p || ''}&id=${c.id}" onclick="return confirm('Refresh Instagram token for ${c.name}?')" style="position:absolute;top:16px;right:90px;background:rgba(0,186,124,0.1);border:1px solid rgba(0,186,124,0.2);color:#00ba7c;border-radius:8px;padding:6px 12px;font-size:12px;text-decoration:none;font-family:inherit">Refresh</a>
