...
async function drawTemplate(quote, author, keyPoints, bgImageBuffer) {
  ...
  if (bgImageBuffer) {
    ...
    // Dark card background so white text always works
  ...
  // ALWAYS white text - background is always dark (either image+overlay or dark fallback)
  const textColor = '#ffffff';
  const subTextColor = 'rgba(255,255,255,0.75)';
  const dividerColor = 'rgba(255,255,255,0.2)';
  const keyPointColor = '#ffffff';
  ...
  ctx.fillStyle = 'rgba(0,0,0,0.4)'; ctx.fillRect(0, S - 58, S, 58);
