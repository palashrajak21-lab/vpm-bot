...
async function webSearch(query) {
  try {
    ...
    if (data.RelatedTopics) {
      data.RelatedTopics.slice(0, 10).forEach(function(t) {
...
// Detect if topic needs web search
function needsSearch(topic) {
  const searchKeywords = ['best', 'top', 'tool', 'app', 'software', 'how to', 'ways to', 'tips for', 'list of', 'free', 'ai tool', 'website', 'platform', 'remove', 'generate', 'create', 'edit', 'compare'];
  const lower = topic.toLowerCase();
  return searchKeywords.some(function(k) { return lower.includes(k); });
}
...
async function generateContent(topic) {
  // Search web if topic is about tools/tips/lists
  let searchContext = '';
  if (needsSearch(topic)) {
    console.log('Searching web for:', topic);
    searchContext = await webSearch(topic);
    if (searchContext) {
      console.log('Got search results, length:', searchContext.length);
    }
  }

  const systemPrompt = searchContext
    ? 'You are an elite Instagram content creator. You have been given web search results to use as reference. Create an accurate, factual post using this real information. For list/tools posts, use the actual tool names from the search results. Respond ONLY with raw JSON. No explanation. No markdown. No backticks. JSON only. Fields: quote (string max 12 words - a catchy hook line for the image), author (empty string), caption (string with actual numbered list of tools/tips from search results, 2-3 paragraphs max 200 words), cta (string), hashtags (string of 10 hashtags). SEARCH RESULTS: ' + searchContext
    : 'You are an elite Instagram content creator. If topic is a person name use their real famous quote. If motivational create a bold statement. Respond ONLY with raw JSON. No explanation. No markdown. No backticks. JSON only. Fields: quote (string max 12 words), author (string or empty), caption (string 2 paragraphs max 150 words), cta (string), hashtags (string of 10 hashtags).';
  ...
  const res = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
    ...
    messages: [
      ...
      { role: 'user', content: 'Create Instagram post about: ' + topic }
    ]
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (code === 9 || code === 10 || code === 13 || code >= 32) cleaned += raw[i];
  }
...
app.post('/webhook', async function(req, res) {
  ...
  try {
    ...
    const searchMsg = needsSearch(text) ? 'Searching the web for real info about: "' + text + '"\n\nPlease wait ~20 seconds...' : 'Generating post about: "' + text + '"\n\nPlease wait ~15 seconds...';
    await sendText(chatId, searchMsg);
