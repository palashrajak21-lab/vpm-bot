...
app.get('/ig-connect', (req, res) => {
  ...
  const redirectUri = PUBLIC_URL + '/ig-callback';
  const igAuthUrl = 'https://api.instagram.com/oauth/authorize?client_id=' + process.env.IG_APP_ID + '&redirect_uri=' + encodeURIComponent(redirectUri) + '&scope=instagram_basic,instagram_content_publish,instagram_manage_comments,instagram_manage_messages&response_type=code&state=' + encodeURIComponent(JSON.stringify({plan: plan||'monthly', payment_id: payment_id||''}));
  ...
  try {
    ...
    const tokenRes = await axios.post('https://api.instagram.com/oauth/access_token', 
      new URLSearchParams({
        ...
        redirect_uri: PUBLIC_URL + '/ig-callback',
        code: code
      }).toString(),
      { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }
    );
