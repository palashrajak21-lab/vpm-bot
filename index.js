...
// Search Unsplash for relevant image (free, no key needed for source API)
async function fetchBackgroundImage(topic) {
  try {
    // Use Unsplash Source API - completely free, no key needed
    const keywords = topic.toLowerCase()
      .replace(/create a post on|create post on|make a post on|post about|latest|news|today/gi, '')
      .trim()
      .split(' ')
      .slice(0, 3)
      .join(',');
    console.log('Fetching image for keywords:', keywords);
    const url = 'https://source.unsplash.com/1080x1080/?' + encodeURIComponent(keywords);
    const res = await axios.get(url, {
      responseType: 'arraybuffer',
      timeout: 10000,
      maxRedirects: 5
    });
    console.log('Background image fetched, size:', res.data.byteLength);
...
app.post('/webhook', async function(req, res) {
  ...
  try {
    ...
    // Fetch relevant background image from Unsplash
    console.log('Fetching background image...');
    const bgImg = await fetchBackgroundImage(text);
