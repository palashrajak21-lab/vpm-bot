...
async function drawTemplate(quote, author, keyPoints, bgImageBuffer) {
  const S = 1080;
  ...
  if (bgImageBuffer) {
    try {
      ...
      // Clip to card shape then draw image
      ctx.save();
      ...
      console.log('BG image draw failed:', e.message);
  ...
  // Always use white text when there's a background image, dark text on white card
  const darkMode = !!bgImageBuffer;
  const textColor = darkMode ? '#ffffff' : '#0f1419';
  const subTextColor = darkMode ? 'rgba(255,255,255,0.75)' : '#536471';
  const dividerColor = darkMode ? 'rgba(255,255,255,0.2)' : '#eff3f4';
  const keyPointColor = darkMode ? '#ffffff' : '#0f1419';
  ...
  try {
    const { loadImage } = require('canvas');
    const logoImg = await loadImage(__dirname + '/logo.png');
    ctx.save();
    ctx.beginPath(); ctx.arc(avX, avY, avR, 0, Math.PI * 2); ctx.clip();
    ctx.drawImage(logoImg, avX - avR, avY - avR, avR * 2, avR * 2);
    ctx.restore();
  } catch(e) {
  ...
  ctx.fillStyle = textColor; ctx.font = 'bold 34px sans-serif'; ctx.textAlign = 'left';
  ...
  ctx.stroke();
  ctx.fillStyle = subTextColor; ctx.font = '26px sans-serif'; ctx.textAlign = 'left';
  ctx.fillText('@visualpromediaofficial', nameX, cy + 126);

  // X logo
  const xs = 28, xx = cx + cw - 68, xy = cy + 56;
  ctx.strokeStyle = textColor; ctx.lineWidth = 4.5; ctx.lineCap = 'round';
  ...
  // Top divider
  ctx.strokeStyle = dividerColor; ctx.lineWidth = 2;
  ...
  if (hasList) {
    ...
    const toolSpacing = Math.min(100, (cy + ch - 220 - toolStart) / keyPoints.length);
  ...
  ctx.fillStyle = subTextColor; ctx.font = '22px sans-serif'; ctx.textAlign = 'center';
...
async function newsSearch(query) {
  try {
    ...
    if (res.data.articles && res.data.articles.length > 0) {
      ...
      const imgArticle = res.data.articles.find(function(a) {
        return a.urlToImage && a.urlToImage.startsWith('http') && !a.urlToImage.includes('placeholder');
      });
      const newsImageUrl = imgArticle ? imgArticle.urlToImage : null;
      console.log('News image URL:', newsImageUrl);
      return { text: context.substring(0, 2000).trim(), imageUrl: newsImageUrl };
    }
...
async function downloadImage(url) {
  try {
    ...
    console.log('Image downloaded, size:', res.data.byteLength);
...
// Fetch background image with strict timeout
async function fetchBackgroundImage(topic, newsImageUrl) {
  const timeout = new Promise(function(resolve) { setTimeout(function() { resolve(null); }, 8000); });
  const fetch = async function() {
    // First priority: use actual news article image
    if (newsImageUrl) {
    ...
    try {
      ...
      const keywords = topic.toLowerCase()
        .replace(/create a post on|create post on|make a post|post about|latest|news|today|give me|show me/gi, '')
        .trim().split(' ').filter(function(w) { return w.length > 2; }).slice(0, 3).join(' ');
      console.log('Pexels search for:', keywords);
      const searchRes = await axios.get('https://api.pexels.com/v1/search', {
        params: { query: keywords, per_page: 3, orientation: 'square' },
        headers: { 'Authorization': PEXELS_KEY },
        timeout: 5000
      });
      if (searchRes.data.photos && searchRes.data.photos.length > 0) {
        const imgUrl = searchRes.data.photos[0].src.large2x || searchRes.data.photos[0].src.large;
        console.log('Pexels image found:', imgUrl.substring(0, 60));
  ...
  try {
    ...
    console.log('Background image result:', result ? 'found' : 'none');
    return result;
  } catch(e) {
    console.log('fetchBackgroundImage error:', e.message);
    return null;
  }
}
...
app.post('/webhook', async function(req, res) {
  ...
  try {
    ...
    console.log('Key points:', JSON.stringify(data.key_points));
    const kp = data.key_points || data.tools || [];
    console.log('Key points for image:', JSON.stringify(kp));
    // Fetch relevant background image — news article image or stock photo
    console.log('Fetching background image...');
    const bgImg = await fetchBackgroundImage(text, data._newsImageUrl || null);
    const imgBuf = await drawTemplate(data.quote || text, data.author || '', kp, bgImg);
