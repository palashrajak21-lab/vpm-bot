...
// Fetch background image — use news article image or Pixabay stock photo
async function fetchBackgroundImage(topic, newsImageUrl) {
  ...
  // Fallback: Pixabay stock photo for general topics
  try {
    const PIXABAY_KEY = process.env.PIXABAY_API_KEY;
    if (!PIXABAY_KEY) return null;
    const keywords = topic.toLowerCase()
      .replace(/create a post on|create post on|make a post|post about|latest|news|today|give me|show me/gi, '')
      .trim().split(' ').filter(w => w.length > 2).slice(0, 3).join('+');
    console.log('Pixabay search for:', keywords);
    const searchRes = await axios.get('https://pixabay.com/api/', {
      params: { key: PIXABAY_KEY, q: keywords, image_type: 'photo', per_page: 3, safesearch: true, min_width: 800 },
      timeout: 8000
    });
    if (searchRes.data.hits && searchRes.data.hits.length > 0) {
      const imgUrl = searchRes.data.hits[0].largeImageURL || searchRes.data.hits[0].webformatURL;
